---
title: About
type: about
sidebar:
  open: true
---

Consolidating my **Tech Knowledge** in a public and [open site](https://github.com/JAlcocerT/JAlcocerT/tree/main/content).

Hope that these guides in form of a website will make the **your Tech Journey** a better experience:

- See the [Docs](https://jalcocert.github.io/JAlcocerT/docs/) for a curated High level view
- See the [blogs](https://jalcocert.github.io/JAlcocerT/blog/) to go deeper into the details and my journey

You can follow me at https://www.youtube.com/@JAlcocerTech

{{< cards >}}
  {{< card link="https://cal.com/jalcocertech/consulting" title="Tech Consulting" image="/blog_img/email/cal-email-stripe.png" subtitle="Questions? Get them solved" >}}
{{< /cards >}}