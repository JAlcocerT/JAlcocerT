...
And a **SEO ready** hugo theme: *lychee is also great for this*
...
See the problems you didnt know your web has: https://webaudit.jalcocertech.com/


### GenBI

I was doing [some tech talk](https://jalcocert.github.io/JAlcocerT/a-diy-boilerplate-to-ship/#the-tech-talk) around this.
...
Bc i have [my mbsd framework 2d/3d](https://jalcocert.github.io/JAlcocerT/design-centric-mbsd/) ready to avoid agents alucinating physics.
...
> PS: including with some [fluid dynamics around combustion](https://github.com/JAlcocerT/mbsd/tree/master/z-fluid-mechanics)
...
Where this fits in the additive-manufacturing pipeline
Phase 	Need 	What we provide
Concept design 	"Will this geometry survive its load history?" 	Multi-body simulation of the loaded chain; load envelope per part
Material selection 	"Will this resonate?" 	Torsional / bending modal analysis; FRF + Campbell diagrams
Process planning 	"Will the part fatigue here?" 	Stress history from the dynamic simulation; fatigue post-processing
Pre-print verification 	"Is this design sound mechanically?" 	Full system simulation including all the dynamic couplings
Iteration 	"Why didn't it work?" 	Open kernel + analytic acceptance criteria — diagnose, don't just observe
...
> [Radial](https://www.youtube.com/watch?v=eCR1P5w2Oik), not [rotary](https://www.youtube.com/watch?v=mjpfIXyWEnw)
...
> See also https://www.youtube.com/@pierrejansen2702
...
```md
 my friend who is a potential lead mechanical engineer is asking me about the tests that we have run against this framework, could we add that to the faq as well? anything else that could be added from the physical standpoint to validate that there are not strange bugs in the 
  ...
  ❯ in the meantime, can you have a look to z-poc-trip-planner-v4? would that have the potential to have ons statically deployable part? like if i want for 2 or 3 locations the historicals?         
    ...
    Open question from my last message: do you want me to add an FAQ section to SALES.md titled "If I wanted to replace Amesim entirely with open source, what would I assemble?" covering the three deployment patterns (OpenModelica route / Julia route / Python-everything route)         
```
...
I couldnt avoid to **make an UI for this**
...
make trip-ship   CF_PROJECT=trip-planner #https://17e286bf.trip-planner-9lt.pages.dev
```

The [result](https://github.com/JAlcocerT/poc/tree/main/go-solar)?
...
> What if someone would take couple sensors, store its data and see how everything works for a particular house?
...
As presented few weeks ago [here](https://jalcocert.github.io/JAlcocerT/youtube-video-as-a-code/) and [here](https://jalcocert.github.io/JAlcocerT/the-ideas-bucket-can-be-empty/#quick-content-creation)
...
They still dont like [to sync expectations](https://jalcocert.github.io/JAlcocerT/setup-bi-tools-docker/#syncing-expectations)?
...
npx wrangler@latest pages deploy dist-why-postmortem \
  ...
  --branch main
#npx wrangler@latest pages deploy dist-why-postmortem-magazine --project-name why-postmortem-checks --branch main
```
...
1. `https://www.kimi.com/` and their kimiclaw...
...
2. https://github.com/Nutlope/hallmark - A design skill for Claude Code, Cursor, and Codex that refuses to look AI-generated.
3. https://github.com/heygen-com/hyperframes - Write HTML. Render video. Built for agents.
...
> built with [rust](https://tinyhumans.gitbook.io/openhuman/developing/architecture)

* https://github.com/earendil-works/pi

The narrative arc across all four: Each post explicitly cross-references the others, so they form a coherent stack story for readers landing on any one of them.
...
Five-design-principle treatment applied to each post (the pattern you liked from NanoClaw/OpenClaw) — surfaces each project's opinionated claims, not just its feature list.

For Hermes, I leaned heavily on the self-improvement angle and the hermes claw migrate "polite-but-clear" positioning as a successor to OpenClaw.
