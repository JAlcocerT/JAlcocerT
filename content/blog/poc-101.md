...
draft: false
tags: ["MVP","Entrepreneur","iPerf3 x Trip v3","JAlcocerTech Branding HUB","Formbricks API"]
...
```sh
cd ./real-estate-calculator #deployed statically
```
...
You can bring any of those html driven ppts at your consulting site for the non-believers [like so](https://consulting.jalcocertech.com/presentations/opco/slides)

![alt text](/blog_img/apps/iperf-ppt.png)

Working with people that are still [trapped at excel](https://jalcocert.github.io/JAlcocerT/blog/biz-grist/) *for some strange reason*?

Chances are that you will need **pptx** exports:

```sh
#slidev can also export pdfs but they are not editable as one created from scratch
...
```sh
#git clone /PBi
#cd ./PBi/executive-dashboard-pptx #a sample pptx w dummy data created w python-pptx :)
```

Remember that you can also do html to pdf:
...
yea, the one [after this event](https://www.linkedin.com/posts/exadel-people_sql-to-insight-pipeline-with-ai-activity-7432721791468367872-E83B)
...
Come on, with html we are able to do even quick videos [like this one](https://youtu.be/zDMjKYQgNUE) to announce your tech talk.

[thx to hyperframes](https://jalcocert.github.io/JAlcocerT/about-motion-graphics/) :)


Have branded assets? Then go pro [via remotionJS](https://github.com/JAlcocerT/jalcocertech-services/tree/master/channel-youtube/genbi-1)
...
{{< callout type="info" >}}
The learnings of this tech talk are stored [here](https://github.com/JAlcocerT/selfhosted-landing/blob/master/y2026-tech-talks/4-iot-to-bigdata/tech-talk/z-learnings.md)
{{< /callout >}}
...
yep, [more remotionJS/hyperframes](https://jalcocert.github.io/JAlcocerT/about-motion-graphics/) again.
...
1. Get your brand assets sorted out. I did so in [my HUB, here](https://github.com/JAlcocerT/jalcocertech-services/tree/master/design).
...
And have started putting together a HUB for my brand assets (finally):
...
I was making some [upgrades to this one](https://jalcocert.github.io/JAlcocerT/quick-weddings-poc/) too...
...
You mentioned being "tired of giving tech knowledge."

This is a sign that you are ready to move from **Stage 2 (Optimization)** to **Stage 3 (Sovereignty).**
...
Nobody cares if you are using: windsurf, cursor, zed, gram, antigravity, [kilo code](https://kilo.ai/open), roo code, claude, codex, gemini cli, Goose Desktop....
...
```sh
# curl -X GET "https://<your-formbricks-host>/api/v1/management/surveys" \
#   -H "Authorization: Bearer <NEW_API_KEY>"
curl --request GET \
  --url "https://app.formbricks.com/api/v1/management/surveys" \
  --header "x-api-key: YOUR_FORMBRICKS_API_KEY"
curl -H "x-api-key:fbk_abcd" "https://app.formbricks.com/api/v1/management/surveys" | jq
```
...
#uv init
uv run main.py whoami
choco install make
#make surveys-json > surveys_output.json #export them all as json
uv run python main.py create survey-sample.json #create one from json
make publish SURVEY_ID= #to make it public :)
```

All included [here](https://github.com/JAlcocerT/jalcocertech-services/tree/master/forms).
...
This is how it looks the vibe coded v3 UI/X:
