...
```mermaid
graph TD
    subgraph Revenue ["Top Line: Growth"]
        P[Price]
        V[Volume]
        P & V --> REV[Total Revenue]
    end

    subgraph Operation ["Middle Line: The COO"]
        REV -->|Minus COGS| GM[Gross Margin]
        GM -->|Minus OpEx| EBITDA[EBITDA]
        EBITDA -->|Minus D&A| EBIT[Operating Income]
    end

    subgraph Final ["Bottom Line: The Owner"]
        EBIT -->|Minus Int/Tax| NP[Net Profit]
    end

    %% Strategic Levers
    RD[R&D/Refinement] -.->|Boosts| P
    MKT[Marketing/Sales] -.->|Boosts| V
    SC[Supply Chain/Procurement] -.->|Shrinks| COGS
    AUTO[Automation/AI] -.->|Shrinks| OPEX
...
graph TD
    ...
    subgraph Efficiency [Operational Efficiency]
        COGS[Unit COGS]
        MKT[CAC / Marketing Spend]
        G_A[Headcount / Admin]
    end

    subgraph Health [Financial Health]
        CASH[Cash on Hand]
        DEBT[Debt Interest]
    end
    ...
    %% The Logic
    P & V --> REV[Total Revenue]
    REV -->|Minus COGS| GP[Gross Profit]
    GP -->|Minus MKT + G_A| EB[EBITDA]
    EB -->|Minus Interest/Tax| NI[Net Profit]
